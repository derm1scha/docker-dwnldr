...
# psutil is optional; fall back to shell if it's missing
try:
...
# --- tiny HTTP file server (threaded) with request logging ---
...
class LoggedFileHandler(SimpleHTTPRequestHandler):
    # Injected: directory via partial(..., directory=FILE_ROOT)
    ...
    def log_message(self, format: str, *args):
        # Override: capture into our ring buffer instead of printing
        entry = {
            "time": _now_iso(),
            "client": self.client_address[0] if self.client_address else "",
            "method": getattr(self, "command", ""),
            "path": getattr(self, "path", ""),
            "code": self._resp_code if self._resp_code is not None else "-",
    ...
    def start(self, port: int, directory: str):
        with self.lock:
            ...
            try:
                ...
                return False, f"bind_failed: {e}"

            t = threading.Thread(target=srv.serve_forever, name=f"file-srv:{port}", daemon=True)
            t.start()
            # quick sanity wait
            time.sleep(0.1)
            self.server, self.thread, self.port = srv, t, port
            return True, None
...
def get_ipv4_for_interface(ifname: str) -> Optional[str]:
    return (
        _get_ipv4_from_psutil(ifname)
        or _get_ipv4_from_ip_cmd(ifname)
        or _get_ipv4_from_ifconfig(ifname)
    )
...
@app.get("/api/ip")
def api_ip():
    return jsonify({"ip": detect_tunnel_ip()})
...
def serve_file(filename: str):
    if not SERVE_FILES:
        abort(404)
    try:
        return send_from_directory(FILE_ROOT, filename, as_attachment=False)
    except FileNotFoundError:
        abort(404)

# --- file-listener control & logs ---
@app.get("/api/file-listener")
def file_listener_status():
    running = file_srv.is_running()
    port = file_srv.port if running else None
...
def file_listener_start():
    ...
    try:
        port = int(port_param or data.get("port") or 8443)
    except Exception:
        port = 8443
...
def index():
    ...
    return render_template(
        "index.html",
        env_static_ip=env_static_ip,
        env_tun_if=env_tun_if,
...
if __name__ == "__main__":
    # One worker is recommended if you run with Gunicorn; for dev, this is fine.
